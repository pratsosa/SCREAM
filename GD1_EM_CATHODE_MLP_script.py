...
# %%
test_loader = torch.load(f'/pscratch/sd/p/pratsosa/{stream}_CATHODE_mlp/linear_test_loader_{name}.pth', weights_only=False)
...
model = EM_LitLinearModel.load_from_checkpoint(f'/pscratch/sd/p/pratsosa/model_checkpoints/{name}/epoch=25.ckpt')
model.eval()
...
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print('Using device:', device)
test_loader_dummy = torch.load(f'/pscratch/sd/p/pratsosa/{stream}_CATHODE_mlp/linear_test_loader_{name}.pth',
                               weights_only=False)
val_loader_dummy = torch.load(f'/pscratch/sd/p/pratsosa/{stream}_CATHODE_mlp/linear_val_loader_{name}.pth',
                                 weights_only=False)
...
# %%
t = Table.read('/pscratch/sd/p/pratsosa/GD-1_gaia_x_decals_stream_prep.fits')
...
# %%
test_ids = get_id_from_data(np.column_stack((test_ra, test_dec, test_pm_ra, test_pm_dec)), ['phi1', 'phi2', 'pm_phi1', 'pm_phi2'], '/pscratch/sd/p/pratsosa/GD-1_gaia_x_decals_stream_prep.fits')
...
# %%
emma_labels = Table.read('..//Data/gd1_members_gaia.fits')
emma_ids = np.array(emma_labels['source_id'])

# %%
emma_test_ids = np.isin(test_ids, emma_ids)

# %%
np.sum(emma_test_ids)

# %%
fig, 
plt.hist2d(test_ra, test_dec, bins=100, cmap='gray_r', alpha=.5)#, label='Background')
plt.scatter(test_ra[emma_test_ids], test_dec[emma_test_ids], s=4, color = '#1f77b4')
...
ax2.hist2d(test_ra[~emma_test_ids], test_dec[~emma_test_ids], bins=100, cmap='gray_r', alpha=.5)  
# Color these points by pmra_error and pmdec_error       
ax2.scatter(test_ra[emma_test_ids], test_dec[emma_test_ids], s=75, color = '#1f77b4')#, label = 'GD1')
...
ax2.set_title('Emma Labels', fontsize=50)
...
# %%
cm = confusion_matrix(emma_test_ids, preds_threshold, normalize=None)

print(f'recall score   : {recall_score(emma_test_ids, preds_threshold):.5f}')
print(f'precision score: {precision_score(emma_test_ids, preds_threshold):.5f}')
print(f'f1 score       : {f1_score(emma_test_ids, preds_threshold):.5f}')
...
# %%
f'/pscratch/sd/p/pratsosa/{stream}_CATHODE_mlp/results_{name}.csv'

# %%
df.to_csv(f'/pscratch/sd/p/pratsosa/{stream}_CATHODE_mlp/results_{name}.csv', index=False)
