...
def crossmatch_gaia_ls(gaia_table: Table, ls_table: Table, max_separation_arcsec: float = 1.0) -> Table:
    """
    ...
    c_gaia = SkyCoord(ra=gaia_table['ra']*u.deg, dec=gaia_table['dec']*u.deg, frame='icrs')
    c_ls = SkyCoord(ra=ls_renamed['ls_ra']*u.deg, dec=ls_renamed['ls_dec']*u.deg, frame='icrs')
    ...
    gaia_table['match_dist_arcsec'][valid_matches] = d2d[valid_matches].to(u.arcsec).value
...
"""
BASE_LS_QUERY = """
SELECT ra, dec, ebv, flux_g, flux_r, flux_z, flux_ivar_g, flux_ivar_r, flux_ivar_z 
FROM ls_dr9.tractor 
WHERE type = 'PSF' AND 
"""
...
def download_and_match_stream(
    ...
    for i in range(Nchunks):
        ...
        tr_full = track_extended[start:stop]
        tr_full = tr_full[::subsample_factor]
        ...
        for j in [-1, 0, 1]:
            poly = galstreams.create_sky_polygon_footprint_from_track(
                tr, mws[stream_track].stream_frame,
        ...
        if chunk_ls_list:
            ...
            merged_ls = unique(merged_ls, keys=['ra', 'dec'], keep='first')
        else:
            merged_ls = Table()

        # 3. Crossmatch
        matched_chunk = crossmatch_gaia_ls(merged_gaia, merged_ls, max_separation_arcsec)
...
def _process_chunk(i, track_extended, step_size, chunk_size, Nchunks, subsample_factor,
    ...
    for j in [-1, 0, 1]:
        poly = galstreams.create_sky_polygon_footprint_from_track(
            tr_full, mws[stream_track].stream_frame,
            width=extension_deg * u.deg,
            phi2_offset=j * extension_deg * u.deg
        )
        chunk_name = f"chunk_{i+1}_{j}"
...
def download_and_match_stream_parallel(
    ...
    max_workers controls concurrency — keep ≤ 8 to avoid DataLab rate limits.
    """
    mws = galstreams.MWStreams(verbose=False, implement_Off=True)
